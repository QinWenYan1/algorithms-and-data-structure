...
- **为什么不直接写后序？** 后序遍历非递归实现需要标记节点访问状态（区分第一次遇到还是第二次遇到），代码复杂；而"根右左"与前序类似，只需调换压栈顺序即可实现，最后 O(n) 反转即可得到后序
    ...
    - **问题来了：节点 1 会被遇到两次**
        ...
        | 1 | `[1]` | 1 入栈 | 第一次遇到 1 |
        | 2 | `[1, 2]` | 2 是左孩子，入栈 | 去左边 |
        | 3 | `[1]` | 2 弹出，输出 `2` | 2 没孩子，可以输出 |
        | 4 | `[1, 3]` | 3 是右孩子，入栈 | 去右边 |
        | 5 | `[1]` | 3 弹出，输出 `3` | 3 没孩子，可以输出 |
        | 6 | `[]` | **1 弹出，输出 `1`** | 左右都完了，终于轮到根 |

    - 看起来走通了？**但程序不知道"步骤 4 之前"和"步骤 6"的区别！**

    - 对程序来说，步骤 3 弹出 2 后，栈顶是 1。这时候程序面临一个**两难**：
        - 1 的左子树已经遍历完了 ✅
        - 但 1 的**右子树还没遍历** ❌
        - 所以此时**绝对不能输出 1**！

    - 步骤 5 弹出 3 后，栈顶又是 1。这时候：
        - 1 的左子树遍历完了 ✅
        - 1 的右子树也遍历完了 ✅
        - 现在**可以输出 1** 了！

    - **同一个节点 1，两次回到栈顶，行为却不同。** 程序怎么知道是"第一次回来"还是"第二次回来"？
