...
1. **先扩容再处理**：
    * 统计数字个数计算最终长度（原长+数字数×5）
    * 一次性`resize`到位，避免中间插入的数据搬移开销。

2. **从后向前双指针**：
    * 快指针读原串末尾，慢指针写新串末尾，反向填充避免覆盖未处理字符，实现原地O(1)空间修改。
    * 从前向后填充就是O(n^2)的算法了，因为每次添加元素都要将添加元素之后的所有元素整体向后移动由于覆盖了为处理字符。

3. **避坑`size_t`下溢**：
    * 循环索引用**有符号`int`** 防止无符号整型`0-1`回绕成`MAX`导致死循环和段错误。
